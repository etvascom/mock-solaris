...
export const creteBookingFromReservation = (person, reservation, incoming?) => {
  ...
  const merchant = JSON.parse(metaInfo).cards.merchant;
  const senderName = merchant.name;
  const senderIBAN = merchant.iban;
  ...
  return {
    ...
    sender_bic: person.account.bic,
    sender_iban: reservation.recipientIBAN || "DE89370400440532013000",
    sender_name: recipientName,
    recipient_bic: process.env.SOLARIS_BIC,
    recipient_name: senderName || SOLARIS_CARDS_ACCOUNT.NAME,
    recipient_iban: senderIBAN || SOLARIS_CARDS_ACCOUNT.IBAN,
    booking_date: moment().utc().format("YYYY-MM-DD"),
    creation_date: moment().utc().format("YYYY-MM-DD"),
    valuta_date: moment().utc().format("YYYY-MM-DD"),
    recorded_at: moment().utc().format(),
